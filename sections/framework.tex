...
Memory faults may affect model weights, activations, and KV caches. We focus on weights because they typically remain resident in GPU memory and are reused throughout decoding. Activations and KV caches have shorter lifetimes, which limits how long their corruption persists. Thus, memory faults in this work refer to persistent weight corruption.
Such weight corruption may occur during model storage or loading. Error-correcting codes can detect or correct many single-bit errors, but uncorrectable multi-bit errors may still cause silent data corruption \cite{hsiao2023silent,papadimitriou2021demystifying,jiao2025large}. To model these errors, vLLM-FI injects random double-bit flips as weights enter the GPU execution path. 
